...
from model.pytorch.core.backbone.resnet_backbone import ResNetBasicBlock as BasicBlock
from model.pytorch.core.backbone.resnet_backbone import ResNetBottleneck as Bottleneck


class HRNetBlock(BaseBlock):
    ...
    def __init__(self, name="semi_supervised_segmentation_block"):
        ...
        self.ocr_extra_final_conv_kernel = 1
        self.stage1_cfg = {
            "NUM_MODULES": 1,
            "NUM_RANCHES": 1,
            "BLOCK": 'BOTTLENECK',
        ...
        self.stage3 = None
        self.transition3 = None
        self.stage4 = None
        self.high_level_ch = None
    ...
    def build(self):
        ...
        self.conv1 = nn.Conv2d(3, 64, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1),
                               bias=False)
        self.bn1 = self.norm2d(64, momentum=self.batch_norm_momentum)
        self.conv2 = nn.Conv2d(64, 64, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1),
                               bias=False)
        self.bn2 = self.norm2d(64, momentum=self.batch_norm_momentum)
        self.relu = nn.ReLU(inplace=self.relu_inplace)
            ...
            if i < num_branches_pre:
                if num_channels_cur_layer[i] != num_channels_pre_layer[i]:
                    transition_layers.append(nn.Sequential(
                        nn.Conv2d(num_channels_pre_layer[i],
                                  ...
                                  bias=False),
                        self.norm2d(
                            num_channels_cur_layer[i], momentum=self.batch_norm_momentum),
                ...
                for j in range(i + 1 - num_branches_pre):
                    ...
                    conv3x3s.append(nn.Sequential(
                        nn.Conv2d(
                            inchannels, outchannels, (3, 3), (2, 2), (1, 1), bias=False),
                        self.norm2d(outchannels, momentum=self.batch_norm_momentum),
                        nn.ReLU(inplace=self.relu_inplace)))
    ...
    def _make_layer(self, block, inplanes, planes, blocks, stride=1):
        ...
        if stride != 1 or inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                ...
                self.norm2d(planes * block.expansion, momentum=self.batch_norm_momentum),
            )
...
class HighResolutionModule(nn.Module):
    def __init__(self, num_branches, blocks, num_blocks, num_inchannels,
                 ...
                 norm2d=nn.BatchNorm2d, batch_norm_momentum=0.1, relu_inplace=True, align_corners=True):
        ...
        self.relu = nn.ReLU(inplace=self.self.relu_inplace)
           ...
           self.num_inchannels[branch_index] != (num_channels[branch_index] *
            ...
            downsample = nn.Sequential(
                nn.Conv2d(self.num_inchannels[branch_index],
                          ...
                          kernel_size=(1, 1), stride=(stride, stride), bias=False),
                self.norm2d(num_channels[branch_index] * block.expansion,
                ...
                if j > i:
                    fuse_layer.append(nn.Sequential(
                        nn.Conv2d(num_inchannels[j],
                                  ...
                                  (0, 0),
                                  bias=False),
                        self.norm2d(num_inchannels[i], momentum=self.batch_norm_momentum)))
                elif j == i:
