...
class SMPLModel(Module):
    ...
    def forward(self, betas, pose, trans, simplify=False):
        ...
        """
        betas = betas.reshape(-1).to(self.device)
        trans = trans.reshape(-1).to(self.device)
        pose = pose.reshape(-1).to(self.device)
...
def load_smpl_model(model_path):
    smpl = SMPLModel(device="cuda:0", model_path=model_path)
    return smpl
